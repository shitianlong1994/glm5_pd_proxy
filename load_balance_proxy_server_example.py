...
def is_retryable_status(status_code: int) -> bool:
    return status_code in RETRYABLE_STATUS_CODES or HTTPStatus.INTERNAL_SERVER_ERROR <= status_code < 600


def get_forwarded_error_headers(response: httpx.Response) -> dict[str, str]:
    return {header: response.headers[header] for header in FORWARDED_ERROR_HEADERS if header in response.headers}


async def create_upstream_http_error(response: httpx.Response) -> UpstreamHTTPError:
    content = await response.aread()
    error = UpstreamHTTPError(
        status_code=response.status_code,
        content=content,
        headers=get_forwarded_error_headers(response),
    )
    await response.aclose()
    return error


def create_client_error_response(error: UpstreamHTTPError) -> Response:
    return Response(
...
def create_bad_gateway_response() -> JSONResponse:
    status_code = HTTPStatus.BAD_GATEWAY.value
    return JSONResponse(
        status_code=status_code,
        content={
            "error": {
                "message": "Failed to communicate with the upstream inference service.",
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if (
                ...
                or not is_retryable_status(response.status_code)
                or attempt == max_retries
            ):
                return response

            logger.warning(
                "Attempt %s failed for %s with retryable HTTP status %s",
                attempt,
                endpoint,
...
async def open_stream_service_response_with_retry(
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            logger.warning(
                "Attempt %s failed for streaming %s with retryable HTTP status %s",
                attempt,
                endpoint,
                response.status_code,
            )
...
async def _handle_select_instance(api: str, req_data: Any, request_length: int):
    ...
    try:
        ...
        if not is_successful_response(response):
            error = await create_upstream_http_error(response)
            raise error
...
async def _handle_completions(api: str, request: Request):
    try:
        ...
        try:
            decoder_response = await open_stream_service_response_with_retry(
            ...
            if not is_successful_response(decoder_response):
                error = await create_upstream_http_error(decoder_response)
                raise error
        except Exception:
        ...
        async def generate_stream():
            nonlocal decoder_response, instance_info
            generated_token = ""
            released_kv = False
            retry_count = 0
            retry = True
            completion_tokens = 0
            current_response = decoder_response
            ...
            try:
                while retry:
                    ...
                    try:
                        async for chunk in current_response.aiter_bytes():
                            ...
                            if stop_reason == "recomputed":
                                ...
                                next_instance_info = await _handle_select_instance(api, req_data, tmp_request_length)
                                proxy_state.release_decoder(instance_info.decoder_idx, instance_info.decoder_score)
                    ...
                    if retry:
                        current_response = await open_stream_service_response_with_retry(
                            instance_info.decoder.client,
                            api,
                            req_data,
                            request_id=instance_info.request_id,
                        ...
                        if not is_successful_response(current_response):
                            error = await create_upstream_http_error(current_response)
                            raise error
        ...
        logger.info("Forwarding upstream HTTP status %s for %s", e.status_code, api)
        proxy_state.request_num -= 1
        return create_client_error_response(e)
    except httpx.RequestError as e:
        logger.error("Failed to communicate with an upstream service for %s: %s", api, e)
        proxy_state.request_num -= 1
        return create_bad_gateway_response()
    except Exception as e:
        import traceback
